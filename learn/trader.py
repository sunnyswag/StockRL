...
class Trader(object):
    def __init__(self, model_name='a2c') -> None:
        ...
        self.sub_trade_dir = "result"
        self.trade_dir = self.get_trade_dir()
    
    def get_trade_dir(self) -> str:
        """获取存储训练结果的路径名"""
        trade_dir = os.path.join(os.getcwd(), self.sub_trade_dir)
        trade_dir = os.path.join(trade_dir, self.model_name)
        self.create_trade_dir(trade_dir)

        return trade_dir
    
    def create_trade_dir(self, trade_dir: str) -> None:
        """创建存储训练结果的文件夹"""
        if not os.path.exists(trade_dir):
            os.makedirs(trade_dir)
            print("{} 文件夹创建成功!".format(self.sub_trade_dir))
        else:
            print("{} 文件夹已存在!".format(self.sub_trade_dir))
    ...
    def get_model(self, agent):
        model = agent.get_model(self.model_name,  
                                ...
                                verbose = 0)
        model_dir = os.path.join(self.sub_train_dir, "{}.model".format(self.model_name))
    ...
    def trade(self) -> None:
        ...
        if model is not None:
            ...
            .format(self.sub_result_dir, self.model_name, self.model_name, self.sub_result_dir))
